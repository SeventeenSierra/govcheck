...
**GovCheck** is an AI-powered NSF PAPPG compliance checker with:
...
| **postgres** | 5432 | Database | `docker-compose -p govcheck exec postgres pg_isready` |
| **redis** | 6379 | Cache | `docker-compose -p govcheck exec redis redis-cli ping` |
...
```bash
# Restart everything
docker-compose -p govcheck restart

# Restart just one service
docker-compose -p govcheck restart web
```
...
2. View logs: `docker-compose -p govcheck logs strands`
