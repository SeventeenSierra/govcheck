# Deployment Guide - GovCheck (Contract Checker)

> **Quick Start for Team Members**: This guide will help you deploy and run the complete GovCheck application with or without AWS AI capabilities.
...
git clone https://github.com/yourusername/govcheck.git
cd govcheck
```
...
docker-compose -p govcheck ps
```

Expected output after 60-90 seconds:
```
NAME                          STATUS             PORTS
govcheck-web-1        Up (healthy)      0.0.0.0:3000->3000/tcp
govcheck-strands-1    Up (healthy)      0.0.0.0:8080->8080/tcp
govcheck-postgres-1   Up (healthy)      0.0.0.0:5432->5432/tcp
govcheck-redis-1      Up (healthy)      0.0.0.0:6379->6379/tcp
govcheck-minio-1      Up (healthy)      0.0.0.0:9000-9001->9000-9001/tcp
```
...
docker-compose -p govcheck logs strands | grep -i "bedrock\|aws"
```
...
# Database
docker-compose -p govcheck exec postgres pg_isready -U postgres
# Expected: postgres:5432 - accepting connections

# Redis
docker-compose -p govcheck exec redis redis-cli ping
# Expected: PONG
```
...
```bash
# All services
docker-compose -p govcheck logs -f

# Specific service
docker-compose -p govcheck logs -f web
docker-compose -p govcheck logs -f strands
```
...
docker-compose -p govcheck logs
...
```bash
cd containers
docker-compose -p govcheck down
```
...
```bash
docker volume ls | grep govcheck
```
