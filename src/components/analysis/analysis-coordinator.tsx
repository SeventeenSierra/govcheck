...
export const AnalysisCoordinator: React.FC<AnalysisCoordinatorProps> = ({
   ...
   */
  const extractTextContent = useCallback(async (content: File | string): Promise<TextExtractionResult> => {
  ...
  const startAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const documentStructure = analyzeDocumentStructure(extractionResult.text);
        ...
        <div className="analysis-status mb-4 p-4 border rounded-lg">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className={`px-2 py-1 rounded text-sm ${
              session.status === AnalysisStatus.COMPLETED ? 'bg-green-100 text-green-800' :
              session.status === AnalysisStatus.FAILED ? 'bg-red-100 text-red-800' :
        ...
        <div className="analysis-results mt-4 p-4 border rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2">{(result.analysisMetadata.processingTime / 1000).toFixed(1)}s</span>
            </div>
          </div>
        </div>
      )}
