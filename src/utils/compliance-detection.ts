...
import { ComplianceIssue, IssueSeverity } from '@/components/analysis/types';
...
export const CRITICAL_VIOLATION_PATTERNS = [
  {
    ...
    remediation: 'Include cybersecurity measures, NIST 800-171 compliance, and basic safeguarding requirements'
  },
  {
...
export const WARNING_VIOLATION_PATTERNS = [
  ...
  },
  {
    ...
    pattern: /(?:will\s+comply|intend\s+to|plan\s+to\s+implement)(?!.*(?:have\s+implemented|currently\s+maintain))/i,
    title: 'Vague Compliance Language',
    description: 'Compliance statements use future tense rather than demonstrating current implementation',
...
export function detectCriticalViolations(text: string): ComplianceIssue[] {
  ...
  const hasCybersecurity = /cybersecurity|nist\s+800-171|information\s+security|basic\s+safeguarding/i.test(text);
  if (!hasCybersecurity) {
    issues.push({
  ...
  const hasBusinessEthics = /business\s+ethics|code\s+of\s+conduct|ethics\s+training|compliance\s+program/i.test(text);
  ...
  const hasAntiTrafficking = /trafficking\s+in\s+persons|human\s+trafficking|forced\s+labor/i.test(text);
 ...
 */
export function findIssueLocations(text: string, searchPattern: RegExp): Array<{
  line: number;
  column: number;
  context: string;
}> {
