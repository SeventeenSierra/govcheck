  ...
  describe('Mock Analysis Results', () => {
    ...
    it('should generate consistent summary statistics', async () => {
      ...
      if (result.data?.issues) {
        ...
        expect(summary.criticalIssues).toBe(issues.filter((i) => i.severity === 'critical').length);
        expect(summary.warningIssues).toBe(issues.filter((i) => i.severity === 'warning').length);
      }
    });
  });
